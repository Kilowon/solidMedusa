...
export default function FocusProductC(props: { item: FeaturedProps['item'] }) {
	...
	return (
		<Show when={true}>
			<div
				class="sm:min-h-[800px] max-w-99svw lg:mb-auto lg:mt-auto  flex items-center  md:justify-center flex-col lg:flex-row lg:space-x-8 mx-2 my-25 sm:my-50 lg:my-auto
		"
			>
				<Show when={getWindowSize().width > 1023}>
					<div class="flex flex-col justify-between max-h-600px min-h-300px max-w-200px  min-w-100px space-y-4 ">
						...
						>
							<img
								src={`${import.meta.env.VITE_DIRECTUS_URL}/assets/${props.item.image_4?.id}?key=200-avif`}
								loading="eager"
								alt="main image"
								class=" object-cover "
							/>
